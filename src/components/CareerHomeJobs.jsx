...
const JobGridCard = ({ job, onView, token }) => {
    const isExpired =
                    ...
                    {token ? (
                        <div className="grid grid-cols-2 gap-2">
                            {/* <motion.button
                                ...
                                className="py-2.5 text-xs font-bold bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl hover:shadow-lg transition-all"
                            >
                                View Details
                            </motion.button> */}
                        ...
                        <Link
                            to="/register"
                            className="flex items-center justify-center gap-2 w-full py-2.5 text-xs font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl hover:shadow-lg transition-all"
                        >
...
const JobTableRow = ({ job, onView, token, idx }) => {
    const isNew = getIsNew(job.postedDate);
    return (
    <motion.tr
    ...
    >
        <td className="px-4 py-3.5 max-w-[200px]">
            ...
            {token && <p className="text-xs text-orange-500 font-medium truncate">{job.org}</p>}
        </td>
        <td className="px-4 py-3.5 hidden md:table-cell">
            {token ? <Badge label={job.category} /> : <span className="text-gray-300">—</span>}
        </td>
        <td className="px-4 py-3.5 text-xs text-gray-600 hidden lg:table-cell">
            {token ? (job.noOfPosts || "—") : <span className="text-gray-300">—</span>}
        </td>
        <td className="px-4 py-3.5 text-xs hidden xl:table-cell">
            {token ? <span className="text-gray-600 line-clamp-1 max-w-[140px] block">{job.qualifications || "—"}</span> : <span className="text-gray-300">—</span>}
        </td>
        <td className="px-4 py-3.5 text-xs hidden xl:table-cell">
            {token ? <span className="text-gray-500">{formatDateDDMMYYYY(job.postedDate)}</span> : <span className="text-gray-300">—</span>}
        </td>
        ...
        <td className="px-4 py-3.5">
            {token ? (
                ...
                <Link
                    to="/register"
                    className="inline-flex items-center justify-center px-3 py-1.5 text-xs font-bold text-white bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg hover:shadow-md transition-all whitespace-nowrap"
                >
                    Register Now
                </Link>  
            )}
...
const JobModal = ({ job, loading, onClose }) => (
    <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
    ...
    >
        ...
        >
            {loading ? (
                ...
                <>
                    ...
                    <div className="p-6 space-y-5">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            {[
                                { label: "Category", value: job?.category, emoji: "📁" },
              ...
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-blue-400 mb-0.5">Location</p>
                <p className="text-sm font-semibold text-gray-800">All India</p>
              </div>
            </div> */}
            ...
            try {
                ...
                if (data.success) {
                    const mapped = (data?.data?.jobs || [])
                        .filter((j) => !String(j?.jobType || "").toLowerCase().includes("intern"))
                        .map((j) => ({
                            id: j._id,
                            title: j.title,
                            org: j.jobSource,
    ...
    const fetchJobDetails = async (id) => {
        ...
        try {
            ...
            if (data.success) {
                const j = data.data;
                setSelectedJob({
                    id: j._id, title: j.title, org: j.jobSource, category: j.jobType,
                    noOfPosts: j.numberOfPosts, age: j.ageRequirement,
                    qualifications: j.qualifications, applyLink: j.applyLink,
                    notificationUrl: j.notificationUrl || j.notificationURL || j.notification_url || "",
                    postedDate: j.postedDate?.split("T")[0],
                    lastDate: j.applicationDeadline?.split("T")[0],
                });
            ...
            <div className="relative z-10 max-w-7xl mx-auto">
                ...
                {!token && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        className="mb-6 flex items-center justify-between gap-4 bg-gradient-to-r from-orange-50 to-yellow-50 border border-orange-200 rounded-2xl px-5 py-4"
                    >
                        <div className="flex items-center gap-3">
                            <span className="text-2xl">🔒</span>
                            <div>
                                <p className="text-sm font-bold text-gray-800">Register for full details & apply</p>
                                <p className="text-xs text-gray-500">View job requirements, deadlines, and apply directly</p>
                            </div>
                        </div>
                        <Link
                            to="/login"
                            className="shrink-0 px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white text-xs font-bold rounded-xl shadow hover:shadow-md transition-all whitespace-nowrap"
                        >
                            Register Now →
                        </Link>
                    </motion.div>
                    ...
                    {viewMode === "grid" && (
                        ...
                        >
                            {loading
                                ...
                                : jobs.map((job, i) => (
                                    <JobGridCard key={job.id} job={job} onView={fetchJobDetails} token={token} />
                                ))}
                        </motion.div>
                    )}
                ...
                <div className="text-center">
                    <motion.div whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}>
                        ...
                        >
                            <span>{token ? "View All Opportunities" : "Login to View All Jobs"}</span>
