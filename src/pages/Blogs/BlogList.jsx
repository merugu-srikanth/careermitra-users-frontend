// BlogList.jsx — Beautiful, Modern, Professional Redesign
...
export default function BlogList() {
  ...
  const fetchBlogs = useCallback(async () => {
    setLoading(true);
    try {
      let url = `${API_BASE}/blogs?page=${currentPage}&limit=7`;
        ...
        <div className="relative max-w-5xl mx-auto px-6 py-20 text-center">
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto relative">
            <div className="relative flex items-center bg-white border-2 border-gray-200 rounded-2xl shadow-sm focus-within:border-orange-400 focus-within:shadow-orange-100 focus-within:shadow-md transition-all duration-300">
              ...
              <button
                type="submit"
                className="m-2 px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold rounded-xl transition-colors"
              >
                Search
              </button>
            </div>
          ...
          <div className="flex items-center justify-center gap-8 mt-10 text-sm text-gray-400">
            ...
            <span className="w-px h-4 bg-gray-200" />
            <span className="flex items-center gap-1.5"><FaEye size={13} className="text-orange-400" /> Free to Read</span>
          </div>
        </div>
      </section>
          ...
          <div className="space-y-10">
                      ...
                      <div className="p-8 lg:p-10 flex flex-col justify-between">
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-xs text-gray-400 mb-5">
                            <span className="flex items-center gap-1.5">
                              <FaCalendarAlt size={11} className="text-orange-400" />
                              {formatDate(featuredBlog.published_at)}
                            </span>
                            <span className="flex items-center gap-1.5">
                              <FaClock size={11} className="text-green-400" />
                              {readTime(featuredBlog.content)} min read
                            </span>
                            <span className="flex items-center gap-1.5">
                              <FaEye size={11} className="text-orange-400" />
                          ...
                          {featuredBlog.tags?.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-6">
                              {featuredBlog.tags.slice(0, 3).map((tag, i) => (
                                <span key={i} className="text-xs px-3 py-1 bg-green-50 text-green-700 border border-green-100 rounded-full font-medium">
                                  #{tag}
                                </span>
                              ))}
                            </div>
                        ...
                        <div className="flex items-center justify-between pt-5 border-t border-gray-100">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              <p className="text-xs font-semibold text-gray-800">{featuredBlog.author_name || 'CareerMitra'}</p>
                              <p className="text-xs text-gray-400">Author</p>
                            </div>
                          </div>
                          <span className="flex items-center gap-2 text-orange-500 text-sm font-bold group-hover:gap-3 transition-all">
                            Read Article <FaArrowRight size={13} />
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            {gridBlogs.length > 0 && (
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-gray-900">Latest Articles</h2>
                  <p className="text-sm text-gray-400 mt-0.5">Fresh insights for your career journey</p>
                </div>
                <span className="hidden md:flex items-center gap-1.5 text-xs text-gray-400 bg-white border border-gray-200 px-3 py-1.5 rounded-full">
                  <FaFire size={11} className="text-orange-400" /> Trending this week
                </span>
              </div>
            )}
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {gridBlogs.map((blog, index) => (
                  <Link
                    key={blog._id}
                    to={`/blog/${blog.slug}`}
                    className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:border-orange-200 hover:shadow-lg hover:shadow-orange-50/60 hover:-translate-y-1 transition-all duration-400 flex flex-col"
                  ...
                  >
                    ...
                    <div className="relative h-48 overflow-hidden bg-gray-100 flex-shrink-0">
                      ...
                      <div className="absolute top-3 left-3">
                        <span className="px-2.5 py-1 bg-white/95 text-orange-600 text-xs font-bold rounded-lg">
                      ...
                      <div className="absolute bottom-3 left-3 right-3 flex items-center gap-2">
                        <span className="flex items-center gap-1 text-white/90 text-xs bg-black/40 px-2 py-0.5 rounded-full backdrop-blur-sm">
                          <FaEye size={10} /> {(blog.views || 0).toLocaleString()}
                        </span>
                        <span className="flex items-center gap-1 text-white/90 text-xs bg-black/40 px-2 py-0.5 rounded-full backdrop-blur-sm">
                          <FaClock size={10} /> {readTime(blog.content)} min
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <div className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                          {blog.author_name?.charAt(0)?.toUpperCase() || 'C'}
                        </div>
                      ...
                      <h3 className="text-base font-bold text-gray-900 leading-snug mb-2 group-hover:text-orange-600 transition-colors line-clamp-2">
                        {blog.title}
                      </h3>
                      ...
                      {blog.tags?.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {blog.tags.slice(0, 2).map((tag, i) => (
                            <span key={i} className="text-xs px-2 py-0.5 bg-gray-50 text-gray-500 border border-gray-100 rounded-full">
                              #{tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-50">
                        <span className="text-orange-500 text-xs font-bold flex items-center gap-1.5 group-hover:gap-2.5 transition-all">
            ...
            {(pagination.totalPages || 1) > 1 && (
              <div className="flex justify-center items-center gap-2 pt-4">
                <button
                  ...
                  className="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-orange-50 hover:border-orange-300 hover:text-orange-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
                  ...
                  num === '...' ? (
                    ...
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl text-sm font-bold transition-all ${
                        (pagination.page || currentPage) === num
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-green-50 hover:border-green-300 hover:text-green-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                >
      ...
      <section className="bg-white border-t border-gray-100 py-20">
        <div className="max-w-2xl mx-auto px-6 text-center">
          <div className="w-14 h-14 bg-orange-50 border border-orange-100 rounded-2xl flex items-center justify-center mx-auto mb-5">
            <FaBell className="text-orange-500" size={22} />
          </div>
          ...
          <p className="text-gray-500 mb-8 max-w-md mx-auto text-sm leading-relaxed">
            Get the latest government job alerts, exam schedules, and career tips delivered straight to your inbox.
          </p>
            <button
            onClick={() => navigate('/register')}
                type="submit"
                className="px-7 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm rounded-xl transition-colors shadow-sm"
                >
                Regitser Now
                </button>
